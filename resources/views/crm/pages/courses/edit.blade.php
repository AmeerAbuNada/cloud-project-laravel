                        ...
                        <div class="card-body">
                            ...
                            <form class="form-horizontal" onsubmit="event.preventDefault(); editProfile(this);">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea name="description" id="description" rows="10" class="form-control">{{ $course->description }}</textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="advisor" id="advisor" class="form-control">
                                            @foreach ($advisors as $advisor)
                                                <option value="{{ $advisor->id }}" @selected($course->user_id == $advisor->id)>
                                                    {{ $advisor->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="is_paid" id="is_paid" class="form-control"
                                            ...
                                            <option value="free" @selected($course->is_paid == 'free')>Free</option>
                                            <option value="paid" @selected($course->is_paid == 'paid')>Paid</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row" @if ($course->is_paid == 'free') style="display: none" @endif
                                    id="cost-container">
...
@section('scripts')
    <script>
        function editProfile(ref) {
            ...
            put("{{ route('courses.update', $course) }}", data, "submit-button", undefined,
                "{{ route('courses.index') }}")
        }
